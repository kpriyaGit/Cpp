...
WeeklyCourse::WeeklyCourse(Poco::JSON::Object::Ptr data) : Course{data}
{

	this->dayOfWeek = Poco::DateTime::DaysOfWeek( data->get("dayOfWeek").convert<int>());
	string time = data->get("startTime").toString();
	this->startTime = this->timeFromString(time);
	time = data->get("endTime").toString();
	this->endTime = this->timeFromString(time);
...
Poco::JSON::Object::Ptr WeeklyCourse::toJson() const
{
	...
	padrs->set("dayOfWeek",to_string(int(dayOfWeek)));

	return padrs;
}


WeeklyCourse* WeeklyCourse::fromJson(Poco::JSON::Object::Ptr data)
{
	WeeklyCourse* wcourse = nullptr;
	if(data->has("dayOfWeek"))
	{
		wcourse = new WeeklyCourse(data);
	}
	return wcourse;
