...
void StudentDb::addStudent1(Student& student)
{
	this->students.emplace(student.getMatrikelNumber(),student);
}
...
{
	Poco::JSON::Object::Ptr pstDB = new Poco::JSON::Object();

	Poco::JSON::Array array_courses;
	for(auto &c: this->courses){
		Poco::JSON::Object::Ptr pourse = c.second->toJson();
		array_courses.add(pourse);
	}

	Poco::JSON::Array array_students;
	for(auto &st: this->students){
			Poco::JSON::Object::Ptr pst = st.second.toJson();
			array_students.add(pst);
		}

	pstDB->set("courses",array_courses);
	pstDB->set("students",array_students);
